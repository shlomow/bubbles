...
def test_connection(get_message):
    bubbles.upload_thought(_SERVER_ADDRESS, _USER_1, _THOUGHT_1)
    message = get_message()
    assert message
...
def test_user_id(get_message):
    ...
    bubbles.upload_thought(_SERVER_ADDRESS, _USER_2, _THOUGHT_1)
    user_id, timestamp, thought = get_message()
...
def test_thought(get_message):
    ...
    bubbles.upload_thought(_SERVER_ADDRESS, _USER_1, _THOUGHT_2)
    user_id, timestamp, thought = get_message()
...
def test_timestamp(get_message):
    ...
    bubbles.upload_thought(_SERVER_ADDRESS, _USER_2, _THOUGHT_2)
    user_id, timestamp, thought = get_message()
    _assert_now(timestamp)


def _test_cli(get_message):
