...
def test_color_image(user, snapshot):
    ...
    out_path = Context.work_path + f'/color_image-{str(timestamp)}.jpg'
    expected = {
...
def test_color_image_cli(tmp_path, user, snapshot):
    data = create_stream(user, [snapshot]).read()
    path = tmp_path / 'snapshot.raw'
    path.write_bytes(data)

    process = subprocess.Popen(
        ['python', '-m', 'bubbles.parsers', 'parse', 'color_image', str(path)],
